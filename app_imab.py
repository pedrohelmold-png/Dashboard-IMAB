"""
app_imab.py — Dashboard Streamlit do carrego do IMA-B (índice completo).

Iniciar com:
    streamlit run app_imab.py --server.port 8502

Diferenças em relação ao app.py (IMA-B 5):
  • Tabelas do banco: carrego_historico_imab + composicao_imab
  • ETL usa flag --index imab
  • Inclui todos os NTN-Bs (sem corte de prazo de 5 anos)
  • Duration média ~9–12 anos (vs ~2–3 anos no IMA-B 5)
"""
...
_CARREGO_TABLE    = "carrego_historico_imab"
_COMPOSICAO_TABLE = "composicao_imab"
...
with st.sidebar:
    st.title("📈 IMA-B")
    st.caption("Carrego diário ex-ante — índice completo")
    st.divider()
    ...
    if st.button("⟳  Atualizar hoje", use_container_width=True, type="primary"):
        with st.spinner("Buscando dados do dia…"):
            result = subprocess.run(
                [sys.executable, "etl.py", "--index", "imab"],
    ...
    n_dias_banco = len(_lch(days=99_999, table=_CARREGO_TABLE))
    ...
    anos_opcoes = {
        ...
        "10 anos (~2520 d.u.)": 2520,
    }
    anos_sel = st.selectbox(
        "Período do backfill",
        options=list(anos_opcoes.keys()),
        index=0,
        help="ANBIMA tem dados históricos de NTN-B desde ~2020. "
             "Quanto mais longo, mais demorado (~0,5s por dia útil).",
    )
    n_backfill = anos_opcoes[anos_sel]
    tempo_est  = round(n_backfill * 0.5 / 60, 1)

    if st.button(f"📥  Baixar histórico ({anos_sel.split()[0]} {anos_sel.split()[1]})", use_container_width=True):
        placeholder = st.empty()
        placeholder.info(f"Buscando {n_backfill} dias úteis na ANBIMA… (~{tempo_est} min)")
        result = subprocess.run(
            [sys.executable, "etl.py", "--index", "imab", "--backfill", "--days", str(n_backfill)],
            capture_output=True, text=True, cwd=_ROOT,
        )
        if result.returncode == 0:
    ...
    st.caption(
        ...
        "**Universo:** todos os NTN-Bs (sem corte de prazo)"
    )
...
# Carga de dados
# ─────────────────────────────────────────────────────────────────
ultimo  = get_ultimo_carrego(table=_CARREGO_TABLE)
hist    = load_carrego_historico(days=periodo_dias, table=_CARREGO_TABLE)
compos  = load_composicao(table=_COMPOSICAO_TABLE)


# ─────────────────────────────────────────────────────────────────
# Header
# ─────────────────────────────────────────────────────────────────
st.title("Carrego do IMA-B")
...
if ultimo:
    ...
    st.warning(
        "Nenhum dado no banco. Clique em **Atualizar dados** no menu lateral "
        "ou execute `python etl.py --index imab --backfill` no terminal."
    )
    st.stop()
...
with c1:
    st.metric(
        label="Taxa Real (IMA-B proxy)",
        value=f"IPCA + {ultimo['ytm_real']:.2f}%",
        help="Média ponderada por duration da taxa indicativa de todos os NTN-Bs.",
    )
...
if hist.empty:
    ...
    # Área sombreada entre carrego e CDI (prêmio)
    if hist["cdi_anual"].notna().any():
        fig.add_trace(go.Scatter(
...
if premio_series.empty:
    ...
    cores_barras = [
        ...
        for v in hist_premio["premio_vs_cdi"]
    ]
...
if compos.empty:
    ...
    with col_contrib:
        ...
        fig3 = go.Figure(go.Bar(
            ...
            hovertemplate=(
                "Venc: %{x}<br>"
                "Contribuição: %{y:.3f} p.p.<extra></extra>"
            ),
        ))
        fig3.update_layout(
...
st.divider()
st.caption(
    "⚠️ **Aviso metodológico:** este dashboard é um **proxy** do IMA-B, não o índice oficial. "
    "Os pesos são calculados por *duration* — não refletem a carteira teórica divulgada pela ANBIMA. "
    "O universo inclui **todos os NTN-Bs** sem corte de prazo (duration média ~9–12 anos). "
